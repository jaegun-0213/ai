...
mylabale
nemds <- data.frame(iris$Petal.Length,mylabale)
head(nemds)
table(nemds$mylabale)
...
#5  소수(prime numbe)는 1과 자기 자신 외에는 나누어 떨어지지 않는 수를 말한다. 2~1000사이의 소수를 출력하는 R코드를 작성하시오
prime <-c()
for(i in 2:1000){
  for(j in 2:i){
    if(i%%j == 0) break
...
apply(mtcars[-5],2,sum)
#(2) mtcars 데이터셋의 열별 최댓값을 출력하시오.
apply(mtcars[-5],2,max)
...
idx4 <- which(iris[, -5]>5.0, arr.ind = T)
idx4
